...
app.get("/searchbyID/:staff_id", async ({params}) => {
  return await db.$queryRaw`SELECT staff_id, "username", staff_name, birthday, gender, staff_phone_number, role, password, email FROM "staff" WHERE "staff_id" = ${Number(params.staff_id)} Limit 1;`;
});
...
app.get("/getStaffName/:staff_id", async ({params}) => {
  return await db.$queryRaw`SELECT staff_name FROM "staff" WHERE role = 'Staff' AND staff_id = ${Number(params.staff_id)}  Limit 1;`;
});

app.get("/getDoctorName/:staff_id", async ({params}) => {
  return await db.$queryRaw`SELECT staff_name FROM "staff" WHERE role = 'Doctor' AND staff_id = ${Number(params.staff_id)} Limit 1;`;
});
